...
const spawnChrome = function(localPath) {
  const proc = spawnProcess(localPath, defaultChromeFlags);
  return proc;
};
...
exports.start = function() {
    ...
    const driver = spawnProcess('bin/chromedriver');
    driver.stderr.on('data', function (line) {
        log('[chrome-driver] `%s`', line)
    });

    driver.stdout.on('data', function (line) {
        log('[chrome-driver] `%s`', line)
    });
